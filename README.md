...
```tsx
import { Canvas, Item, MoveHandle, ResizeHandle, ScaleHandle, RotateHandle } from 'headless-canvas';
...
| `onDragStart` / `onDragEnd` | `(id, kind) => void` | — | Drag lifecycle, with the drag kind (`'move' \| 'resize' \| 'scale' \| 'rotate'` or a custom kind). |
...
| `features` | Declarative affordances: `<MoveHandle />`, `<ResizeHandle />`, `<ScaleHandle />`, `<RotateHandle />`, or your own feature components. |
| `className` / `style` | Pass-through hooks for consumer CSS. |
...
| `ResizeHandle` | `direction?` — `n/s/e/w/ne/nw/se/sw` (default `'se'`), `cursor?` | Resizes that edge/corner; `n`/`w` handles keep the opposite edge fixed. Min size 8 (configurable via constraints). No-op on auto-sized items. |
| `ScaleHandle` | `anchor?` — `'nw' \| 'ne' \| 'sw' \| 'se' \| 'center'` (default `'ne'` — top-right, the reserved scale position), `cursor?` | Proportional scaling (`newH = newW × h/w`); the corner opposite the anchor stays fixed (`'ne'` keeps the bottom-left corner put), `'center'` keeps the item center fixed. Clamped to the canvas bounds. No-op on auto-sized items. |
...
| Hook | Returns |
|---|---|
| `useCanvas()` | `{ width, height, scale, selectedId, hoveredId, select, getItem, updateItem, snap, toLogical }` — throws outside `<Canvas>`. `getItem`/`updateItem` read/write the store; `snap` snaps to the configured grid; `toLogical` maps client → logical coordinates. |
...
| Feature anchor | `data-feature="move\|resize\|scale\|rotate"`, `data-direction="se"` etc. |
...
**Convention for handle visibility** — the styled kit hides handles by default and fades them in with `[data-hovered="true"]` / `[data-selected="true"]` (plus `:focus-within` for keyboard users). The kit's reserved corner positions are pure CSS pins: move at `left/top`, resize at `right/bottom`, scale at `right/top`, rotate above top-center.
...
- **Bounds** — items can never leave the canvas: move, resize, scale and keyboard moves clamp to the canvas edges (override individual edges with `constraints`). Programmatic writes (`updateItem`, controlled `items`) are not clamped — consumers own those.
...
Multi-select/grouping · snap-to-other-item guides · pinch zoom · custom drag shapes (paths) · in-canvas text editing · a spatial index for very large canvases.
